#!/usr/bin/python
'''
Example usage of Waymo Pytorch dataloader
Inspired from "PointRCNN" RCNN dataloader 
'''
...
from lib.datasets.waymo_fresh_dataset import WaymoDataset
import lib.utils.kitti_utils as kitti_utils
import lib.utils.roipool3d.roipool3d_utils as roipool3d_utils
from lib.config import cfg


class WaymoRCNNDataset(Dataset):
    def __init__(self, root_dir, npoints=16384, split='train', classes='VEHICLE', mode='TRAIN', random_select=True,
        ...
        self.dataset = WaymoDataset(root_dir=root_dir, split=split)
...
#                 self.sample_id_list = self.framenum_to_idx
                num_samples = 190 * self.dataset.num_files
                self.logger.info('Load testing samples from %s' % root_dir + " " + split)
    ...
    def __len__(self):
        if cfg.RPN.ENABLED:
            return self.dataset.total_frames#190 * self.dataset.num_files
        elif cfg.RCNN.ENABLED:
            if self.mode == 'TRAIN':
    ...
    def get_rpn_sample(self, index):
        ...
        if pts_lidar.ndim > 3:
            ...
            pts_intensity = np.ones(len(pts_lidar))
        
        if cfg.GT_AUG_ENABLED and self.mode == 'TRAIN':
            # all labels for checking overlapping
            all_gt_obj_list = self.filtrate_dc_objects(curr_frame_labels)
...
#             ret_pts_intensity = pts_intensity[choice] - np.mean(pts_intensity[choice])  # translate intensity to [-0.5, 0.5]
#             ret_pts_intensity = -0.5 + (ret_pts_intensity - np.min(ret_pts_intensity))/(np.max(ret_pts_intensity) - np.min(ret_pts_intensity))*1
        else:
...
#             ret_pts_intensity = pts_intensity - 0.5
        ...
        if self.mode == 'TEST':
            ...
            sample_info['frame_ts'] = frame.timestamp_micros 
            return sample_info
        
        gt_obj_list = self.filtrate_objects(curr_frame_labels)
